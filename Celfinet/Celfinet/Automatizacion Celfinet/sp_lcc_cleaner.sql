...
-- **********************************************************************************************************
	...
	into #ddbb
	from sys.databases
	where name like 'FY1718%'
	and name like '%BURGOS%'
	OR name like '%JEREZ%'
...
begin
		...
		begin
			...
			begin
				exec ('
					use '+@ddbb+'
					if (select name from sys.sysusers where name = ''SREPSQLBMVF\celfinet1'') is not null
					begin
						DROP USER [SREPSQLBMVF\celfinet1]
					end
					if (select name from sys.sysusers where name = ''SREPSQLBMVF\celfinet2'') is not null
					begin
						DROP USER [SREPSQLBMVF\celfinet2]
					end
					if (select name from sys.sysusers where name = ''SREPSQLBMVF\VFCEL'') is not null
					begin
						DROP USER [SREPSQLBMVF\VFCEL]
					end
					if (select name from sys.sysusers where name = ''SREPSQLBMVF\VFCEL2'') is not null
					begin
