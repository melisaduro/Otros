use FY1617_Data_MRoad_A5_H2_2

	
	declare @ciudad as varchar (256)='A5-BAD-R11'
...
insert into [dashboard].dbo.Th_UL_LTE
	select  @ciudad, 
			v.mnc,
...
insert into [dashboard].dbo.Th_DL_LTE
	select  @ciudad, 
			v.mnc,
			SUM(case when (v.direction='Uplink' and v.TestType='UL_NC' and ISNULL(v.Throughput,0)>384) then 1 else 0 end) as 'Count_Throughput_384k_NC_UL',	
			SUM(case when (v.direction='Uplink' and v.TestType='UL_CE' and ISNULL(v.Throughput,0)>384) then 1 else 0 end) as 'Count_Throughput_384k_CE_UL',
			SUM(case when (v.direction='Uplink' and v.TestType='UL_NC') then 1 else 0 end) as 'Count_Throughput_NC_UL',	
			SUM(case when (v.direction='Uplink' and v.TestType='UL_CE') then 1 else 0 end) as 'Count_Throughput_CE_UL'
		from 
			TestInfo t,
			@All_Tests_UL a,
			Lcc_Data_HTTPTransfer_UL v
